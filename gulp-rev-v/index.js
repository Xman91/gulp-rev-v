...
var revPathV = require('rev-path-v');
...
function getManifestFile(opts, cb) {
	file.read(opts.path, opts, function (err, manifest) {
		...
		// console.log(opts.path,manifest);
		cb(null, manifest);
	});
}
...
function transformFilename(file) {
	...
	file.path = modifyFilename(file.path, function (filename, extension) {
		...
		filename = extIndex === -1 ?
			revPathV(filename, file.revHash) :
			revPathV(filename.slice(0, extIndex), file.revHash) + filename.slice(extIndex);
	...
	return through.obj(function (file, enc, cb) {
		...
		sourcemaps.forEach(function (file) {
			...
			if (pathMap[reverseFilename]) {
				...
				file.path = revPathV(file.path.replace(/\.map$/, ''), hash) + '.map';
			} else {
				transformFilename(file);
			}
	...
	return through.obj(function (file, enc, cb) {
		...
		getManifestFile(opts, function (err, manifestFile) {
			...
			if (opts.merge && !manifestFile.isNull()) {
				var oldManifest = {};
				// console.log(JSON.parse(manifestFile.contents.toString()));
